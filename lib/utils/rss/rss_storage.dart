...
class RssStorage {
  ...
  /// 获取RSS订阅信息
  Future<RSS> getRss(String key) async => RSS.fromJson(json.decode((await _instance).getString('rss_list@$key')!));
  /// 移除RSS订阅信息
  removeRss(String key) async => (await _instance).remove('rss_list@$key');

  Future<List<String>?> getRssList() async => (await _instance).getStringList('rss_list');
  void _setRssList(List<String> list) async => (await _instance).setStringList('rss_list', list);
}